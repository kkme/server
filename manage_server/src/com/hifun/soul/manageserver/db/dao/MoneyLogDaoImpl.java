...
public class MoneyLogDaoImpl extends AbstractLogDao implements IMoneyLogDao {
	...
	@Override
	public List<MoneyLog> queryMoneyLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<MoneyLog> resultList = new ArrayList<MoneyLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("currency_type,");
			sbUnion.append("currency_num,");
		...
		try {
			...
			while (rs.next()) {
				MoneyLog.Builder logBuilder = MoneyLog.newBuilder();
				...
				logBuilder.setCurrencyType(rs.getInt(14));
				logBuilder.setCurrencyNum(rs.getInt(15));
				logBuilder.setAfterNum(rs.getInt(16));						
				logBuilder.setCreateTime(rs.getLong(17));
