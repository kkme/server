 ...
 *
 */
public class OnlineTimeLogDaoImpl extends AbstractLogDao implements IOnlineTimeLogDao {
	
	@Override
	protected String getTablePartName(){
		return "online_time_log";
	}
	
	@Override
	public List<OnlineTimeLog> queryOnlineTimeLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<OnlineTimeLog> resultList = new ArrayList<OnlineTimeLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("minute,");
			sbUnion.append("total_minute,");
			sbUnion.append("last_login_time,");
			sbUnion.append("last_logout_time,");
		...
		sb.append("param,");
		sb.append("minute,");
		sb.append("total_minute,");
		sb.append("last_login_time,");
		sb.append("last_logout_time,");
		...
		try {
			...
			while (rs.next()) {
				OnlineTimeLog.Builder logBuilder = OnlineTimeLog.newBuilder();
				...
				logBuilder.setMinute(rs.getInt(14));
				logBuilder.setTotalMinute(rs.getInt(15));
				logBuilder.setLastLoginTime(rs.getLong(16));
				logBuilder.setLastLogoutTime(rs.getLong(17));	
