...
public class RechargeLogDaoImpl extends AbstractLogDao implements IRechargeLogDao  {
	...
	@Override
	public List<RechargeLog> queryRechargeLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<RechargeLog> resultList = new ArrayList<RechargeLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("recharge_num,");	
			sbUnion.append("crystal_num,");
			sbUnion.append("crystal_reward_num,");
			sbUnion.append("before_num,");
			sbUnion.append("after_num,");
			sbUnion.append("exchange_rate,");
		...
		sb.append("reason,");
		sb.append("param,");
		sb.append("recharge_num,");	
		sb.append("crystal_num,");		
		sb.append("crystal_reward_num,");		
		sb.append("before_num,");		
		sb.append("after_num,");		
		...
		try {
			...
			while (rs.next()) {
				RechargeLog.Builder logBuilder = RechargeLog.newBuilder();
				...
				logBuilder.setRechargeNum(rs.getInt(14));	
				logBuilder.setCrystalNum(rs.getInt(15));		
				logBuilder.setCrystalRewardNum(rs.getInt(16));		
				logBuilder.setBeforeNum(rs.getInt(17));		
				logBuilder.setAfterNum(rs.getInt(18));		
				logBuilder.setExchangeRate(rs.getInt(19));		
				logBuilder.setCreateTime(rs.getLong(20));
