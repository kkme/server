 ...
 * 
 */
public class ChatDaoImpl extends AbstractLogDao implements IChatLogDao {
	...
	@Override
	public List<ChatLog> queryChatLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<ChatLog> resultList = new ArrayList<ChatLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("dest_passport_id,");
			sbUnion.append("dest_role_name,");
			sbUnion.append("chat_type,");
			sbUnion.append("content,");
		...
		sb.append("reason,");
		sb.append("param,");
		sb.append("dest_passport_id,");
		sb.append("dest_role_name,");
		sb.append("chat_type,");
		sb.append("content,");
		...
		try {
			...
			while (rs.next()) {
				ChatLog.Builder logBuilder = ChatLog.newBuilder();
				...
				logBuilder.setDestPassportId(rs.getLong(14));
				logBuilder.setDestRoleName(rs.getString(15));
				logBuilder.setChatType(rs.getInt(16));
				logBuilder.setContent(rs.getString(17));
