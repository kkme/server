 ...
 *
 */
public class ItemLogDaoImpl extends AbstractLogDao implements IItemLogDao {
	...
	@Override
	public List<ItemLog> queryItemLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<ItemLog> resultList = new ArrayList<ItemLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("bag_type,");
			sbUnion.append("bag_index,");
			sbUnion.append("template_id,");
			sbUnion.append("item_u_u_i_d,");
			sbUnion.append("count,");
		...
		try {
			...
			while (rs.next()) {
				ItemLog.Builder logBuilder = ItemLog.newBuilder();
				...
				logBuilder.setBagType(rs.getInt(14));
				logBuilder.setBagIndex(rs.getInt(15));
				logBuilder.setTemplateId(rs.getInt(16));
				logBuilder.setItemUUID(rs.getString(17));
				logBuilder.setCount(rs.getInt(18));
