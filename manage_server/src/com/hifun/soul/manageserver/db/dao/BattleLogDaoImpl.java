 ...
 * 
 */
public class BattleLogDaoImpl extends AbstractLogDao implements IBattleLogDao {
	@Override
	...
	@Override
	public List<BattleLog> queryBattleLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<BattleLog> resultList = new ArrayList<BattleLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("stage_id,");
			sbUnion.append("result,");
			sbUnion.append("item_ids,");
		...
		try {
			...
			while (rs.next()) {
				BattleLog.Builder logBuilder = BattleLog.newBuilder();
				...
				logBuilder.setStageId(rs.getInt(14));
				logBuilder.setResult(rs.getInt(15));
				logBuilder.setItemIds(rs.getString(16));
