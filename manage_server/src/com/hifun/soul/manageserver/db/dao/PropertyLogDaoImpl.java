 ...
 *
 */
public class PropertyLogDaoImpl extends AbstractLogDao implements IPropertyLogDao {
	...
	@Override
	public List<PropertyLog> queryPropertyLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<PropertyLog> resultList = new ArrayList<PropertyLog>();		
		...
		for (int i = 0; i <= daySpan; i++) {
			...
			sbUnion.append("property_key,");
			sbUnion.append("property_change,");							
		...
		try {
			...
			while (rs.next()) {
				PropertyLog.Builder logBuilder = PropertyLog.newBuilder();
				...
				logBuilder.setPropertyKey(rs.getInt(14));
				logBuilder.setPropertyChange(rs.getString(15));						
