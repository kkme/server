...
public class SkillPointLogDaoImpl extends AbstractLogDao implements ISkillPointLogDao {
	
	@Override
	protected String getTablePartName(){
		return "skill_point_log";
	}
	
	@Override
	public List<SkillPointLog> querySkillPointLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<SkillPointLog> resultList = new ArrayList<SkillPointLog>();		
		...
		try {
			...
			while (rs.next()) {
				SkillPointLog.Builder logBuilder = SkillPointLog.newBuilder();
