 ...
 *
 */
public class HoroscopeLogDaoImpl extends AbstractLogDao implements IHoroscopeLogDao {
	
	@Override
	protected String getTablePartName(){
		return "horoscope_log";
	}
	
	@Override
	public List<HoroscopeLog> queryHoroscopeLogList(Date beginDate, Date endDate, String sqlWhere, int start, int maxCount) {
		List<HoroscopeLog> resultList = new ArrayList<HoroscopeLog>();		
		...
		try {
			...
			while (rs.next()) {
				HoroscopeLog.Builder logBuilder = HoroscopeLog.newBuilder();
				...
				logBuilder.setHoroscopeId(rs.getInt(16));
