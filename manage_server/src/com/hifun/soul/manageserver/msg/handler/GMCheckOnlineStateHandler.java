...
import com.hifun.soul.manageserver.msg.GMCheckOnlineState;
import com.hifun.soul.proto.services.Services.CheckCharacterOnlineStateResponse;

@Component
public class GMCheckOnlineStateHandler implements IMessageHandlerWithType<GMCheckOnlineState> {
	@Autowired
	...
	@Override
	public void execute(GMCheckOnlineState message) {
		RpcCallback<CheckCharacterOnlineStateResponse> callback = manager
		...
		callback.run(CheckCharacterOnlineStateResponse.newBuilder()
				.setIsOnline(message.isOnline()).build());
	}
