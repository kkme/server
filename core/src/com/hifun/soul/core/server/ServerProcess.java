...
import com.hifun.soul.core.msg.filter.FloodByteAttackFilter;
import com.hifun.soul.core.msg.filter.FloodCmdAttackFilter;
...
public class ServerProcess {
	...
	private final AbstractIoHandler<? extends ISession> messageIoHandler;
	...
	public ServerProcess(String ip, String name, String ports,
			...
			AbstractIoHandler<? extends ISession> ioHandler, String misIps) {
		this(ip, name, ports, messageRecognizer, ioHandler, 1, misIps);
	}
	 ...
	 */
	public ServerProcess(String ip, String name, String ports,
			IMessageRecognizer messageRecognizer, AbstractIoHandler<? extends ISession> ioHandler,
			int ioProcessorPerPort, String misIps) {
		if (name == null || ports == null || messageRecognizer == null
				|| ioHandler == null) {
			throw new IllegalArgumentException(
					CommonErrorLogInfo.ARG_NOT_NULL_EXCEPT);
		}
	...
	public void start() throws IOException {
		...
		for (int i = 0; i < portList.length; i++) {
			...
			// 添加防洪水攻击过滤器
			cfg.getFilterChain().addFirst("floodCheck_byte",
					new FloodByteAttackFilter(this.misIps));
			cfg.getFilterChain().addFirst("floodCheck_cmd",
					new FloodCmdAttackFilter(this.misIps));
			...
			IoAcceptor acceptor = new SocketAcceptor(this.perPortIoProcessor,
					new NewThreadExecutor());
			try {
				acceptor.bind(_address, messageIoHandler, cfg);
			} catch (Exception e) {
				throw new RuntimeException("Bind address ["
						+ _address.toString() + "] fail", e);
			}
			this.acceptors.add(acceptor);

			log("Listening on " + _address + ", io prcessors:"
