...
public class QueueMessageExecutorProcessor implements IMessageProcessor {
	...
	public QueueMessageExecutorProcessor(IMessageHandler<IMessage> messageHandler) {
	...
	public void process(IMessage msg) {
		...
		try {
			...
			try {
				...
				if (msg instanceof BaseSessionMessage) {
					// 如果在处理消息的过程中出现了错误,将其断掉
					final BaseSessionMessage<?> imsg = (BaseSessionMessage<?>) msg;
