...
public class GameDecoder extends ProtocolDecoderAdapter {
	...
	/** 数据处理缓存* */
	private ByteBuffer readBuf = ByteBuffer
			.allocate(IMessage.DECODE_MESSAGE_LENGTH);

	/** 消息识别器 * */
	private final IMessageRecognizer recognizer;

	public GameDecoder(IMessageRecognizer recog, short tgwLength) {
		if (recog == null) {
	...
	private String readTGWString(ByteBuffer buf, String charset) {
		short len = LocalConstants.TGW_LENGTH;
	...
	public void decode(IoSession session, ByteBuffer in,
		...
		// FIXME: crazyjohn 由于腾讯的TGW接入搞得这里很乱，到时候重构下; TGW判断
		if (in.get(0) == 't') {
			String tgwString = readTGWString(in, "GBK");
			System.out.println(tgwString);
		}
		// 先将最近接收的数据全部放到缓冲中
		put(in);
		try {
					...
					try {
						...
						if (m.read()) {
							...
							if (logger.isWarnEnabled()) {
								logger.warn(ErrorsUtil
										.error(CommonErrorLogInfo.MSG_PRO_ERR_READ_FAIL,
												"Ignore", m.getTypeName()));
							}
							break;
						}
	...
	private void expand(int newCapacity) {
		...
		if (newCapacity > 1024 * 32) {
			...
			System.err.println(String.format("Warn:GameDecoder.expand(%dK)",
					newCapacity / 1024));
		}
	}
