package com.hifun.soul.core.orm.annotation;
...
import com.hifun.soul.core.annotation.IAnnotationFilter;
import com.hifun.soul.core.orm.IEntity;
import com.hifun.soul.core.orm.annotation.exception.EntityPackageNotFoundException;
...
public class EntityScanner {
	 ...
	 */
	public List<Class<? extends IEntity>> getEntityListByAnnotationFilter(
			IAnnotationFilter filter) {
	...
	private void scanAllEntityClassInPackage() {
		...
		for (String entityClassName : entityClassNameList) {
			try {
				Class<? extends IEntity> entityClass = (Class<? extends IEntity>) Class
						.forName(entityClassName);
				if (IEntity.class.isAssignableFrom(entityClass)) {
					entityList.add(entityClass);
					logger.info("[EntityAnnotationScanner#scanAllEntityClassInPackage]:Entity '"
							+ entityClassName + "' loaded successfully!");
				}
	...
	private List<String> listAllClassNamesInPackage() {
		...
		URL entityPackageURL = Thread.currentThread().getContextClassLoader()
				.getResource(convertPackageToFolderPath(entityPackage));
		if (entityPackageURL == null) {
			logger.warn("[EntityAnnotationScanner#listAllClassNamesInPackage]:"
					+ "Scanner did not locate the entity package!");
			return list;
		}
		if ("file".equals(entityPackageURL.getProtocol())) {
			...
			for (File entityFile : entityFiles) {
				if (entityFile.isFile()
						&& entityFile.getName().endsWith(".class")) {
					String entityClassName = entityPackage
							+ "."
							+ entityFile.getName().substring(0,
									entityFile.getName().length() - 6);
					list.add(entityClassName);
					logger.info("[EntityAnnotationScanner#listAllClassNamesInPackage]:"
							+ entityClassName + " found!");
				}
			}
			...
			String jarPath = entityPackageURL.getPath().substring(5,
					entityPackageURL.getPath().indexOf("!"));
			try {
				// 将路径中出现的+号提前过滤掉
				jarPath = jarPath.replaceAll("[+]", "%2b");
				JarFile jarFile = new JarFile(URLDecoder.decode(jarPath,
						"UTF-8"));
				Enumeration<JarEntry> entries = jarFile.entries();
				while (entries.hasMoreElements()) {
					String entryName = entries.nextElement().getName();
					if (entryName
							.startsWith(convertPackageToFolderPath(entityPackage))
			...
			throw new UnsupportedOperationException(
					"[EntityAnnotationScanner#listAllClassNamesInPackage]:"
							+ "Scanner can not handle the protocol of  the entity package!");
		}
		return list;
	}
