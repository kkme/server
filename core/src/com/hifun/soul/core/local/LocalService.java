package com.hifun.soul.core.local;
...
import com.hifun.soul.core.util.PM;
...
public class LocalService {
	...
	private static final Logger logger = Loggers.LOCAL_LOGGER;
	/** 平台接口配置 */
	...
	public static final class SynLocalService {
		 ...
		 * @return
		 */
		public AddWallowInfoResponse addWallowInfo(long passportId,
				String trueName, String idCard, String ip) {
			final long _begin = System.nanoTime();
			// 填写防沉迷的认证信息
			AddWallowInfoResponse _response = this.synLocalHandler
					.addWallowInfo(passportId, trueName, idCard, ip);
			LocalService.collectRequestUseTime(_response, _begin);
			return _response;
		}
		 ...
		 *         &nbsp;&nbsp;999：系统异常，查询操作不成功<br>
		 */
		public GetGoodsResponse getGoods(long userId, String roleId, String ip,
				int id, int largessId) {
			final long _begin = System.nanoTime();
			GetGoodsResponse _response = this.synLocalHandler.getGoods(userId,
		 ...
		 *         &nbsp;&nbsp;999：系统异常，查询操作不成功<br>
		 */
		public QueryGoodsResponse queryGoods(long userId, String roleId,
				String ip) {
			final long _begin = System.nanoTime();
		 ...
		 *         &nbsp;&nbsp;999：系统异常，兑换操作不成功<br>
		 */
		public TransferResponse transfer(long userId, String roleId, int money,
				String ip) {
			final long _begin = System.nanoTime();
			TransferResponse _response = this.synLocalHandler.transfer(userId,
					roleId, money, ip);
			LocalService.collectRequestUseTime(_response, _begin);
		 ...
		 *         &nbsp;&nbsp;999：系统异常，操作不成功<br>
		 */
		public long queryOnlineTime(long userId) {
			final long _begin = System.nanoTime();
			QueryOnlineTimeResponse _queryResponse = this.synLocalHandler
					.queryOnlineTime(userId);
		 ...
		 *         &nbsp;&nbsp;999：系统异常，退出操作不成功<br>
		 */
		public LogoutResponse reqPlayerLogout(long userId, String ip) {
			final long _begin = System.nanoTime();
			LogoutResponse _response = this.synLocalHandler.logout(userId, ip);
			LocalService.collectRequestUseTime(_response, _begin);
	...
	public static final class AsyncLocalService {
		 ...
		 *            用户消耗贵重物品时的客户端IP地址。
		 */
		public void reportTransRecord(String orderId, long userId,
				String roleId, int money, String goodSid, String ip) {
			final long _beginTime = System.nanoTime();
			try {
				this.asyncLocalHandler.transRecordReport(orderId, userId,
		...
		public void reportPlayer(String ip, String reporterPassName,
				long reporterPassId, long reporterCharId,
				String reporterCharName, int reporterLevel, long reportTime,
				String playerPassName, long playerPassId, long playerCharId,
			...
			try {
				this.asyncLocalHandler.reportPlayerReport(ip, reporterPassName,
						reporterPassId, reporterCharId, reporterCharName,
						reporterLevel, reportTime, playerPassName,
						playerPassId, playerCharId, playerCharName,
	...
	private static void collectRequestUseTime(IResponse response, long beginTime) {
		...
		if (response == null || response.getUseTime() == 0) {
			// PIProbeCollector.collect(ProbeName.RPC, ProcessResult.FAIL,
