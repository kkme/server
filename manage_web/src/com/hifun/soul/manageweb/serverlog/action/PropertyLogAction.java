...
public class PropertyLogAction extends BaseAction {
	/**
	 * 
	 */
	...
	public String queryPropertyLogList() throws ServiceException{
	    LogRpcService.BlockingInterface serverLogService = Managers.getServiceManager().getServerLogService();
	    QueryPropertyLogRequest.Builder request  = QueryPropertyLogRequest.newBuilder();
	    ManageLogger.log(ManageLogType.QUERY_PROPERTY_LOG_LIST,PermissionType.QUERY_PROPERTY_LOG_LIST, "", true);
	    ...
	    QueryPropertyLogResponse response = serverLogService.queryPropertyLog(null, request.build());
	    propertyLogs = response.getPropertyLogsList();
