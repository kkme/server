...
<%@page import="com.hifun.soul.proto.services.LogServices.BasicPlayerLog"%>
...
<body>
	<s:form action="queryBasicPlayerLogList">
		<table>
			<tr>
				<td colspan="22">开始日期 <sx:datetimepicker name="beginDate"
						displayFormat="yyyy-MM-dd"></sx:datetimepicker> 结束日期 <sx:datetimepicker
						name="endDate" displayFormat="yyyy-MM-dd"></sx:datetimepicker>
					服务器id <s:textfield name="serverId" theme="simple"></s:textfield>
					账号id <s:textfield name="accountId" theme="simple"></s:textfield>
					账号名称 <s:textfield name="accountName" theme="simple"></s:textfield>
					角色id <s:textfield name="characterId" theme="simple"></s:textfield>
					角色名称 <s:textfield name="characterName" theme="simple"></s:textfield>
			...
			<%
				Object logObject = request.getAttribute("basicPlayerLogs");
					if (logObject != null) {
						@SuppressWarnings("unchecked")
						List<BasicPlayerLog> playerLogs = (List<BasicPlayerLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (BasicPlayerLog log : playerLogs) {
			%>
			<tr>
				<td><%=index++%></td>
				...
				<td><%=log.getIp()%></td>
				<td><%=log.getCrystal()%></td>
				<td><%=log.getCoin()%></td>
				<td><%=log.getExperience()%></td>
				<td><%=log.getEnergy()%></td>
			...
			%>
			<tr>
				<td colspan="7">记录总数[<s:property value="pagingInfo.totalCount" />]
				</td>
