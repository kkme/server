...
<%@page import="com.hifun.soul.proto.services.LogServices.MoneyLog"%>
...
<body>
	<s:form action="queryMoneyLogList">
		<table>
			<tr>
				<td colspan="19">开始日期
				...
				货币类型
				<s:textfield name="currencyType" theme="simple" ></s:textfield>
				记录原因id
				<s:textfield name="logReason" theme="simple" ></s:textfield>			
			...
			<%
				Object logObject = request.getAttribute("moneyLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<MoneyLog> moneyLogs = (List<MoneyLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (MoneyLog log : moneyLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getCurrencyType() %></td>
				<td><%=log.getCurrencyNum() %></td>
				<td><%=log.getAfterNum()%></td>
