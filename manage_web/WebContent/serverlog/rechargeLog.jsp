...
<%@page import="com.hifun.soul.proto.services.LogServices.RechargeLog"%>
...
<body>
	<s:form action="queryRechargeLogList">
		<table>
			<tr>
				<td colspan="22">开始日期
			...
			<%
				Object logObject = request.getAttribute("rechargeLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<RechargeLog> rechargeLogs = (List<RechargeLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (RechargeLog log : rechargeLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getRechargeNum() %></td>
				<td><%=log.getCrystalNum() %></td>
				<td><%=log.getCrystalRewardNum() %></td>
				<td><%=log.getBeforeNum() %></td>
				<td><%=log.getAfterNum() %></td>
				<td><%="1:"+log.getExchangeRate() %></td>				
