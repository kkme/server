...
<%@page import="com.hifun.soul.proto.services.LogServices.OnlineTimeLog"%>
...
<body>
	<s:form action="queryOnlineTimeLogList">
		<table>
			<tr>
				<td colspan="20">开始日期
			...
			<%
				Object logObject = request.getAttribute("onlineTimeLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<OnlineTimeLog> onlineTimeLogs = (List<OnlineTimeLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (OnlineTimeLog log : onlineTimeLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getMinute()/60 %></td>
				<td><%=log.getTotalMinute()/60 %></td>
				<td><%=dfDateTime.format(new Date(log.getLastLoginTime()))%></td>
				<td><%=dfDateTime.format(new Date(log.getLastLogoutTime()))%></td>
