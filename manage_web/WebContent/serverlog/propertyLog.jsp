...
<%@page import="com.hifun.soul.proto.services.LogServices.PropertyLog"%>
...
<body>
	<s:form action="queryPropertyLogList">
		<table>
			<tr>
				<td colspan="18">开始日期
			...
			<%
				Object logObject = request.getAttribute("propertyLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<PropertyLog> propertyLogs = (List<PropertyLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (PropertyLog log : propertyLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=CharacterHelper.getCharacterPropertyName(log.getPropertyKey())%></td>
				<td><%=log.getPropertyChange() %></td>
