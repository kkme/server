...
<%@page import="com.hifun.soul.proto.services.LogServices.HoroscopeLog"%>
...
<body>
	<s:form action="queryHoroscopeLogList">
		<table>
			<tr>
				<td colspan="20">开始日期
			...
			<%
				Object logObject = request.getAttribute("horoscopeLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<HoroscopeLog> horoscopeLogs = (List<HoroscopeLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (HoroscopeLog log : horoscopeLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getBagType() %></td>
				<td><%=log.getBagIndex() %></td>
				<td><%=log.getHoroscopeId()%></td>
