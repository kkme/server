...
<%@page import="com.hifun.soul.proto.services.LogServices.FriendLog"%>
...
<body>
	<s:form action="queryFriendLogList">
		<table>
			<tr>
				<td colspan="20">开始日期
			...
			<%
				Object logObject = request.getAttribute("friendLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<FriendLog> friendLogs = (List<FriendLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (FriendLog log : friendLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getFriendPassportId() %></td>
				<td><%=log.getFriendPassportName() %></td>
				<td><%=log.getFriendRoleId() %></td>
				<td><%=log.getFriendRoleName() %></td>
