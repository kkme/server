...
<%@page import="com.hifun.soul.proto.services.LogServices.ItemLog"%>
...
<body>
	<s:form action="queryItemLogList">
		<table>
			<tr>
				<td colspan="21">开始日期
			...
			<%
				Object logObject = request.getAttribute("itemLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<ItemLog> itemLogs = (List<ItemLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (ItemLog log : itemLogs) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getTemplateId()%></td>
				<td><%=log.getItemUUID() %></td>
				<td><%=log.getCount() %></td>
