...
<%@page import="com.hifun.soul.proto.services.LogServices.ExperienceLog"%>
...
<body>
	<s:form action="queryExperienceLogList">
		<table>
			<tr>
				<td colspan="18">开始日期
			...
			<%
				Object logObject = request.getAttribute("experienceLogs");
					if (logObject != null ) {
						@SuppressWarnings("unchecked")
						List<ExperienceLog> logList = (List<ExperienceLog>) logObject;
						DateFormat dfDateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						int index = 1;
						for (ExperienceLog log : logList) {
			%>
			<tr>
				<td><%=index++ %></td>
				...
				<td><%=log.getChangeNum() %></td>
				<td><%=log.getAfterChangeNum()%></td>
