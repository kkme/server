 ...
 * 
 */
public class GiveSelfAmendPropertyBuffEffector extends AbstractBuffEffector
	...
	public EffectResult effect(IBattleUnit attacker, IBattleUnit target,
		...
		if (success) {
			IBuff addBuff = factory.createCommonAmendRoundBuff(attacker,
					buffType, tempTemplate, round, propValue, amendType);
			context.getBuffManager().addBuff(addBuff);
			// 给自己上的buff在播放效果前,先播放一次buff效果;
			addBuff.beforeAction();
		}
