...
public class BossWarCallback extends BaseBattleCallback {
	private int hp;
	private int encourageRate;

	public BossWarCallback(Human challenger, int hp, int encourageRate) {
		super(challenger);
		this.hp = hp;
	...
	public void onChallengerWin(Human challenger, IBattleUnit beChallenged) {
		GameServerAssist.getBossWarService().damageBoss(challenger, getDamage(beChallenged), true);
	}

	@Override
	public void onChallengerLose(Human challenger, IBattleUnit beChallenged) {
		GameServerAssist.getBossWarService().damageBoss(challenger, getDamage(beChallenged), false);
	}
	...
	public void beforeBattleStart(Human challenger, IBattleUnit beChallenged) {
		...
		if(encourageRate != 0){
			int skillAttack = challenger.getHumanPropertiesManager().getIntPropertySet(PropertyType.LEVEL2_PROPERTY).getPropertyValue(Level2Property.ATTACK);
			skillAttack = (int) (encourageRate/SharedConstants.DEFAULT_FLOAT_BASE*skillAttack);
			int gemAttack = challenger.getHumanPropertiesManager().getIntPropertySet(PropertyType.LEVEL2_PROPERTY).getPropertyValue(Level2Property.GEM_ATTACK);
			gemAttack = (int) (encourageRate/SharedConstants.DEFAULT_FLOAT_BASE*gemAttack);
			challenger
					.getBattleContext()
					.getBattleProperty()
					.amendProperty(
			...
			challenger
					...
					.amendProperty(
							AmendMethod.ADD,
							PropertyType.genPropertyKey(Level2Property.GEM_ATTACK,
	...
	public BattleType getBattleType() {
		return BattleType.PVE_BOSS_WAR;
	}
