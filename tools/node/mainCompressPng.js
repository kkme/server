/**
 * 基于nodejs的调用tinypng云服务进行png文件的压缩工具;<br>
 * 注意免费版的接口单月处理量不可超过500;<br>
 * @author crazyjohn;
 */
 ...
 */
var key = "0rYc_Mjt07J2xLRhjAF2t3Lh_xb9JOjj";
...
for (var i = 0; i < unitDirs.length; i++) {
	// 必须要是目录
	if (!fs.statSync(srcDir + unitDirs[i]).isDirectory()) {
		continue;
	}
	if (unitDirs[i] < unitBgein || unitDirs[i] > unitEnd) {
		continue;
	}
	var pngFiles = fs.readdirSync(srcDir + unitDirs[i] + "/");
	console.log(unitDirs[i] + "'s png file size: " + pngFiles.length);
	// 压缩文件
	pngFiles.forEach(function(file) {
		...
		options.method = "POST";
		// 输入输出流
		var input = fs.createReadStream(srcDir + unitDirs[i] + "/" + file);
		var output = fs.createWriteStream(srcDir + unitDirs[i] + "/out_" + file);
